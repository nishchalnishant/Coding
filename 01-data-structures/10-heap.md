...
- **Insert `🎯 T2`**: Append, then bubble up — swap with parent while heap property violated. O(log N).
...
1. **Find K Pairs with Smallest Sums `⚡ T1`**:
...
2. **Kth Smallest Element in a Sorted Matrix `⚡ T1`**:
...
> [!TIP]
> **Sliding Window Median (hard follow-up) `⚡ T1`**: Use two heaps + **lazy deletion**. Track invalid elements (those that have slid out of the window) in a counter. When they surface at the top of a heap during a pop, discard them instead. This avoids O(N) re-heapification per step.

#### Common Variants & Twists
1. **Sliding Window Median `⚡ T1`**:
...
> [!IMPORTANT]
> **The Click Moment**: "Tasks with **deadlines**" — OR — "assign work based on **priority**" — OR — "**Task Scheduler `⚡ T1`** with cooldown". Sort by deadline/profit, then use a heap to dynamically pick the best available option.
...
### Medium
- **K Closest Points to Origin `⚡ T1`** — Max-heap of size K on squared distance (avoid sqrt).
- **Top K Frequent Elements `⚡ T1`** — Count with map; min-heap on `(freq, key)` of size K, or bucket sort.
- **Reorganize String `⚡ T1`** — Max-heap by frequency; place most frequent not equal to previous.
- **Task Scheduler `⚡ T1`** — Math: `(max_freq - 1) * (n + 1) + count_of_max_freq`, capped at `len(tasks)`.
- **Find Median from Data Stream `⚡ T1`** — Two heaps with rebalancing.

### Hard
- **Merge K Sorted Lists `⚡ T1`** — K-way merge; handle empty lists; tie-break with list index.
- **Sliding Window Median `⚡ T1`** — Two heaps + lazy deletion counter.
- **IPO (Maximize Capital) `⚡ T1`** — Sort by capital; max-heap of unlocked profits.
- **Smallest Range Covering K Lists** — K-way merge tracking `[min, max]` of current window.
- **Furthest Building `⚡ T1`** — Greedy: min-heap of climbs assigned ladders; fall back to bricks.
...
| :--- | :--- | :--- | :--- | :--- |
| **[Merge K Sorted Lists](../02-algorithms/20-problem-deep-dives.md#merge-k-sorted-lists) `⚡ T1`** | K-Way Merge (Min-Heap) | "Smallest among K heads" | Min-heap `(val, list_id, node)` | Tie-break: include `list_id` as second element to avoid comparing nodes. |
| **[Kth Largest Element](../02-algorithms/20-problem-deep-dives.md#kth-largest-element) `⚡ T1`** | "Rank without full sort" | QuickSelect O(N) avg or min-heap-K | QuickSelect mutates array; heap is cleaner for streams. Confirm if K can equal N. |
| **[Top K Frequent](../02-algorithms/20-problem-deep-dives.md#top-k-frequent-elements) `⚡ T1`** | "K by frequency" | Count + min-heap-K, or bucket sort | Bucket sort is O(N) — prefer when K not given or K = N. |
| **[Find Median from Stream](../02-algorithms/20-problem-deep-dives.md#find-median-from-data-stream)** | "Median as data arrives" | Two heaps: max-lo + min-hi | Even count → average of tops; ensure `lo` stays ≥ `hi` in size. |
| **K Closest Points `⚡ T1`** | "Nearest K in Euclidean" | Max-heap-K on `x²+y²` | Use squared distance — avoids `sqrt` and float precision. |
| **IPO `⚡ T1`** | "Max capital after K projects" | Sort by capital; max-heap of profits | Projects unlock incrementally as capital grows — pointer into sorted list. |
| **Reorganize String `⚡ T1`** | "No two adjacent same char" | Max-heap by freq; place, then swap back | Impossible if `max_freq > (n+1)//2`. Handle tie when two chars have equal freq. |
| **[Task Scheduler](../02-algorithms/20-problem-deep-dives.md#task-scheduler) `⚡ T1`** | "Min time with cooldown n" | `(max_f-1)*(n+1) + count_max_f`, cap at len | The cap: `max(formula, len(tasks))` — when variety fills the idle slots. |
| **Sliding Window Median `⚡ T1`** | "Median in moving window of size k" | Two heaps + lazy deletion dict | Lazy delete: only remove from top when it surfaces, not immediately. |
...
| **Furthest Building You Can Reach `⚡ T1`** [M] | "Use bricks/ladders optimally to climb" | Min-heap of ladder-used jumps; swap smallest ladder for bricks when bricks run out | Greedily save ladders for largest jumps; min-heap tracks the smallest ladder jump currently used. |
...
| **K Closest Points to Origin `⚡ T1`** [M] | "Efficiently find K closest" | Max-heap of size K; pop if heap size > K | Alternatively: QuickSelect O(N) average — no sorting needed. Max-heap gives O(N log K). |
...
| **Ugly Number II `⚡ T1`** [M] | "Nth ugly number (factors only 2, 3, 5)" | Min-heap; push `n*2`, `n*3`, `n*5` on each pop; deduplicate | Use a visited set or three-pointer DP approach to avoid duplicates in heap. |
| **Find K Pairs with Smallest Sums `⚡ T1`** [M] | "K smallest pairs (one from each sorted array)" | Min-heap seeded with `(nums1[i]+nums2[0], i, 0)`; expand column per row | Only seed first column; on pop `(i, j)`, push `(i, j+1)` — avoids O(N²) initial population. |
| **Trapping Rain Water II `⚡ T1`** [H] | "3D trapped water in matrix" | Min-heap BFS from borders; propagate water level inward | Level at each cell = `max(border_height, parent_level)`; process border cells first. |
...
- **Insert `🎯 T2`**: Push to max-heap, pop max-heap top and push to min-heap.
...
- **Find `⚡ T1`**: If odd total size, median is the top of max-heap. If even, median is the average of both tops.
