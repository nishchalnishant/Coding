...
└── At L3, interviewer expects O(1) time to name the right DS given constraints
│
...
└── Step 5: Confirm complexity fits constraint (n ≤ 10^5 → need O(n log n) or better)
│
...
- **Root problem:** Every problem has a dominant operation, and choosing a DS that makes that operation slow (O(n) instead of O(1) or O(log n)) produces a solution that is correct but fails under time constraints.
...
- **Why it works:** Each DS encodes a structural invariant (heap property, BST ordering, trie prefix path, LIFO stack discipline) that makes its target operation cheap by definition.
- **Where it breaks:** Over-engineering (segment tree when prefix sum suffices) wastes interview time; under-engineering (array where hash needed) produces TLE — both are failure modes.

# The Data Structure Tree: Your Tactical Toolkit

> [!NOTE]
> This file is a **reference / meta document** — read it once, use it as a lookup during revision.
> Not a coding practice file.

Think of this as your "Map of the Hardware." When you're in an interview and the problem feels overwhelming, come back here. The key to L3 mastery isn't knowing every data structure; it's knowing **which tool to grab for which constraint.**
...
- **List Reversal Variants** — Segmented or conditional list reversal requiring careful pointer rewiring.
    - *Patterns:* Reverse Nodes in k-Group, Reverse Linked List II.
...
- **Classic DS Design** — Implementing standard data structures with specific performance constraints.
...
- **Basic Traversal** — Foundation for exploring connectivity and node relationships.
    - *BFS:* Shortest path in unweighted graph, Level-order expansion, Multi-source BFS.
...
- **Shortest Paths** — Optimizing routes between nodes based on edge weight constraints.
    - *Patterns:* Dijkstra (Non-negative weights), Bellman-Ford (Handles negative weights / limited hops).
...
- **Grid Patterns** — Applying graph algorithms (DFS/BFS) to coordinate-based grid systems.
    - *Patterns:* Number of Islands (DFS/BFS), Word Search (Backtracking), Rotting Oranges (Multi-source BFS).

## 8. Advanced Structures (L3 Core)

- **Trie (Prefix Tree)** — Optimized string storage and prefix retrieval; essential for dictionary-based problems.
    - *Patterns:* Implement Trie, Word Search II (Trie + DFS), Design Add and Search Words.
- **Disjoint Set Union (DSU / Union-Find)** — Tracking connected components and dynamic merging with near-O(1) performance.
    - *Patterns:* Redundant Connection, Accounts Merge, Number of Provinces.
...
4. **Optimize**: Check if a more advanced structure (like DSU or a Heap) can reduce time complexity from O(N²) to O(N log N) or O(α(N)).

## Flashcards

**What data structure should you reach for when you need to maintain a dynamic median from a stream of numbers?** #flashcard
**Two Heaps**: A Max-Heap for the lower half of the numbers, and a Min-Heap for the upper half. Keeping them balanced within size difference ≤ 1 allows retrieval of the median in O(1) time.
...
For **sparse graphs** (where the number of edges E ≪ V²). An Adjacency List requires only O(V + E) space and allows O(degree) iteration over neighbors, whereas a matrix requires O(V²) space and O(V) neighbor iteration.
