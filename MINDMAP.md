...
└── bit-manipulation.md
    ...
    │       → Preprocess grid for start position, key count. BFS with state space O(R · C · 2^K). Goal: keys_mask == (1 << num_keys) - 1
...
└── string-algorithms.md
    ...
    │   │   → Pre-compute BASE^(m-1) mod MOD. On hash match, verify character-by-character to rule out collisions. Double hashing (two independent mod values) reduces false positive probability to ~1/(p₁·p₂)
