...
> [!IMPORTANT]
> **The Click Moment**: "**Sort a linked list**" — OR — "stable sort with **guaranteed O(N log N)** worst case". Merge sort is the only comparison sort that achieves all three simultaneously.
...
> **Quick Reference:** `T(n) = aT(n/b) + O(n^d)`. If `d > log_b(a)` → O(n^d); if `d == log_b(a)` → O(n^d log n); if `d < log_b(a)` → O(n^log_b(a)).
