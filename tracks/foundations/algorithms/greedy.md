...
**[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#jump-game-ii)** (minimum jumps): greedy “furthest in current jump range” — extend `end` of current level, when `i == reach` increment jumps and set `reach = furthest`.
...
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#jump-game-ii)** | **Greedy BFS levels:** `end` of current jump range, `furthest` max reach; when `i==end`, increment jumps, `end=furthest`. | **Not** DP for standard statement—greedy optimal; **prove** why extending furthest in current range works. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#non-overlapping-intervals)** | Sort by **end**; keep last picked end; if `start < lastEnd` skip/remove. | **Sort by start** fails (counterexample); **erase** minimum intervals = same as max non-overlapping count. |
...
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#gas-station)** | If `sum(gas) < sum(cost)` no start; else start from 0 with running tank; reset start when tank < 0. | **Uniqueness** of valid start when total surplus ≥ 0; **circular** array linearized by reset. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#task-scheduler)** | Idle formula: `(max_freq-1)*(n+1) + num_tasks_with_max_freq`, **capped** at `len(tasks)`. | **Cap** means cooling fits inside available slots; **heap** simulation alternative. |
