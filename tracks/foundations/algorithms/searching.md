...
- **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#median-of-two-sorted-arrays)**: Binary search on partition in smaller array; balance left sizes and compare max_left ≤ min_right.
...
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#search-in-rotated-sorted-array)** | Find **sorted** half by comparing `nums[lo]`, `nums[mid]`; check if target in sorted range; else other half. | **Duplicates** at `lo==mid==hi` → shrink `hi--` or **O(n)** worst. **Pivot** unknown—two binary searches alternative. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#find-minimum-in-rotated-sorted-array)** | Compare `mid` to `right`: if `nums[mid] > nums[right]`, min in right half; else left. | **No duplicates** simplifies; **with duplicates** similar shrink trick. |
...
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#split-array-largest-sum)** | `valid(mid)` = need ≤ `m` subarrays with max sum `mid`; **greedy** fill subarray until exceed. | **Binary search** on answer range `[max(A), sum(A)]`. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#median-of-two-sorted-arrays)** | Binary search **partition** on shorter array so left half has `(m+n+1)//2` elements and `max(left) ≤ min(right)`. | **`i=0` or `j=0`** sentinels −∞/∞; **even** vs **odd** total length. |
