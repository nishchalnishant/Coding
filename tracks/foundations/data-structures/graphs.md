...
- [Problem Details](../../google-sde2/PROBLEM_DETAILS.md#number-of-islands) (grid DFS/BFS)
- Flood Fill
- [Problem Details](../../google-sde2/PROBLEM_DETAILS.md#rotting-oranges) (multi-source BFS)
- Course Schedule / Course Schedule II (topo + cycle)
- [Problem Details](../../google-sde2/PROBLEM_DETAILS.md#clone-graph) (graph traversal + map)
- Surrounded Regions (border flood fill)

### Hard/Stretch (still common at Google)
- [Problem Details](../../google-sde2/PROBLEM_DETAILS.md#word-ladder) (BFS; bidirectional BFS follow-up)
- [Problem Details](../../google-sde2/PROBLEM_DETAILS.md#alien-dictionary) (topo; prefix invalid case)
...
|----------|------------|----------------------|
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#number-of-islands)** | DFS/BFS flood-fill from each unvisited `1`; count components. | **4 vs 8** neighbors; recursion depth on large grid—use stack/queue. |
| **Flood Fill** | BFS/DFS recolor connected component starting at `(sr,sc)` with original color. | Don’t loop if `newColor == oldColor`. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#rotting-oranges)** | Multi-source BFS from all rotten; minutes = BFS levels; track fresh count. | If fresh remain unreachable → `-1`. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#course-schedule)** | Build directed edges “prereq → course”; topo; cycle means impossible. | Edge direction is the #1 bug: `a,b` means **b before a**. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#clone-graph)** | DFS/BFS with map `old → clone`; connect neighbors using the map. | Cycles require you to create clone before traversing neighbors. |
| **Surrounded Regions** | Flood from border `O` to mark safe; flip remaining `O` to `X`. | Don’t BFS from every interior `O`—too slow. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#word-ladder)** | BFS; neighbors are one-letter edits in set; first reach end is shortest. | Bidirectional BFS follow-up; remove visited words early. |
| **[Problem Details](../../google-sde2/PROBLEM_DETAILS.md#alien-dictionary)** | Build edges from first mismatch in adjacent words; topo all letters. | Prefix invalid: `"abc"` before `"ab"` is impossible. |
