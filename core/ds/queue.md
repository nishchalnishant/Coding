...
- **[Word Ladder](../algo/problem-deep-dives.md#word-ladder)** — BFS; neighbors = one-letter edits in word set; remove visited words from set immediately.
- **[Sliding Window Maximum](../algo/problem-deep-dives.md#sliding-window-maximum)** — Monotonic deque of indices; decreasing values; front = max.
...
- **[Cheapest Flights Within K Stops](../algo/problem-deep-dives.md#cheapest-flights-within-k-stops)** — BFS with state `(node, stops_used)`; or Bellman-Ford with K+1 relaxations.
...
| **[Word Ladder](../algo/problem-deep-dives.md#word-ladder)** | "Minimum transformation steps" | BFS; each word's neighbors = one-letter edits in word set | Remove word from set when visited — prevents revisit and cycles. Bidirectional BFS halves explored nodes for follow-up. |
| **[Sliding Window Maximum](../algo/problem-deep-dives.md#sliding-window-maximum)** | "Max in every k-window in O(N)" | Deque of indices; pop back while `nums[back] < nums[i]`; pop front if expired | Each index enqueued/dequeued once → O(N). Store indices, not values, for window expiry check. |
...
| **[Cheapest Flights K Stops](../algo/problem-deep-dives.md#cheapest-flights-within-k-stops)** | "Shortest path with at most K intermediate nodes" | BFS level = stops; or Bellman-Ford K+1 rounds | Standard Dijkstra doesn't bound stops. Need state `(cost, node, stops)` and prune when `stops > K`. |
