...
### Medium
- [House Robber](../problem-deep-dives.md#house-robber) — Linear DP; space-optimize to O(1).
- [Longest Increasing Subsequence](../problem-deep-dives.md#longest-increasing-subsequence) — O(N²) DP or O(N log N) patience sort (binary search).
- [Coin Change](../problem-deep-dives.md#coin-change) — Unbounded knapsack; forward iteration.
- [Word Break](../problem-deep-dives.md#word-break) — Linear DP with set lookup; `dp[i]` = can s[:i] be segmented.
...
### Hard
- [Edit Distance](../problem-deep-dives.md#edit-distance) — 2D LCS variant; space-optimize to O(N).
- [Burst Balloons](../problem-deep-dives.md#burst-balloons) — Interval DP; k = last balloon burst.
...
| :--- | :--- | :--- | :--- |
| **[House Robber](../problem-deep-dives.md#house-robber)** | "No adjacent selection" | `dp[i] = max(dp[i-1], val + dp[i-2])` | Space-optimize to 2 vars; handle `len=1` separately. |
| **[LIS](../problem-deep-dives.md#longest-increasing-subsequence)** | "Strictly increasing order" | O(N²) DP or O(N log N) patience sort | **O(N log N)** is often expected at SDE-3; patience sort uses `bisect_left`. |
| **[Word Break](../problem-deep-dives.md#word-break)** | "Partition string into dictionary words" | `dp[i]` = can segment `s[:i]` | Use a **set** for O(1) lookup; nested loop is O(N²) without it. |
| **[Edit Distance](../problem-deep-dives.md#edit-distance)** | "Transform string via ops" | 2D DP on prefix pairs | Initialize `dp[0][j]=j` and `dp[i][0]=i`; space-optimize to 1D. |
...
| **[Burst Balloons](../problem-deep-dives.md#burst-balloons)** | "Last operation in range" | Interval DP; k = last to burst | k is the **last** balloon, not first — this is what makes subproblems non-overlapping. |
