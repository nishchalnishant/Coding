...
> [!note] L3 Reference Index — not a tracker
> A lookup table of every problem covered in the `coding/` walkthroughs, grouped by canonical topic. Each problem appears exactly once.
> **Track your own solve progress elsewhere** (LeetCode list, spreadsheet, Anki). This file is for "which problems exist, and where is the writeup" — read it once, then use it as a lookup.

**125 problems** — 48 `⚡ T1` · 77 `🎯 T2`. If you are short on time, solve every `⚡ T1` before touching any `🎯 T2`.

- Pattern logic and gotchas → [`03-patterns/TOPIC_QUESTIONS_LOGIC_AND_TRICKS.md`](03-patterns/TOPIC_QUESTIONS_LOGIC_AND_TRICKS.md)
- Study order and pacing → [`coding/l3-google-roadmap.md`](coding/l3-google-roadmap.md)
...
- [3Sum `⚡ T1`](coding/data-structures/01-array.md#3sum) — Find all unique triplets `[a, b, c]` in `nums` with `a + b + c = 0`. No duplicate triplets [M]
...
- [Longest Duplicate Substring `⚡ T1`](coding/algorithms/02-string-algorithms.md#longest-duplicate-substring--t1) — Find the longest substring appearing at least twice. Binary search on length + Rabin-Karp rolling hash [H]
- [Repeated String Match `🎯 T2`](coding/algorithms/02-string-algorithms.md#repeated-string-match--t2) — Minimum times to repeat `a` so that `b` is a substring of it; KMP or Z on the bounded repetition [M]
- [Longest Happy Prefix `🎯 T2`](coding/algorithms/02-string-algorithms.md#longest-happy-prefix--t2) — Longest proper prefix that is also a suffix. The answer *is* `lps[-1]` from KMP [H]
- [Shortest Palindrome `🎯 T2`](coding/algorithms/02-string-algorithms.md#add-minimum-characters-to-make-a-string-palindrome) — Add minimum characters to the front to make `s` a palindrome; run KMP on `s + '#' + rev(s)` [H]
